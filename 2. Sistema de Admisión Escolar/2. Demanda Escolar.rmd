...
a1_18 <-left_join(a1_18, simce_4b_2017%>%
  ...
  rename(gse4b = cod_grupo, ins_lec4b = palu_eda_ins_lect4b_rbd, ins_mat4b = palu_eda_ins_mate4b_rbd)%>%
    mutate(
      ins_lec4b = ifelse(ins_lec4b <= 25, 0,
      ifelse(ins_lec4b > 25 & ins_lec4b <= 50, 1, 2)),
      ins_mat4b = ifelse(ins_mat4b <= 25, 0,
      ifelse(ins_mat4b > 25 & ins_mat4b <= 50, 1, 2))), by = 'rbd')
...
a1_18 <-left_join(a1_18, simce_2m_2017%>%
  ...
  rename(gse2m = cod_grupo, ins_lec2m = palu_eda_ins_lect2m_rbd, ins_mat2m = palu_eda_ins_mate2m_rbd)%>%
    mutate(
      ins_lec2m = ifelse(ins_lec2m <= 25, 0,
      ifelse(ins_lec2m > 25 & ins_lec2m <= 50, 1, 2)),
      ins_mat2m = ifelse(ins_mat2m <= 25, 0,
      ifelse(ins_mat2m > 25 & ins_mat2m <= 50, 1, 2))), by = 'rbd')
...
a1_19 <-left_join(a1_19, simce_6b_2018%>%
  ...
  rename(gse6b = cod_grupo, ins_lec6b = palu_eda_ins_lect6b_rbd, ins_mat6b = palu_eda_ins_mate6b_rbd)%>%
    mutate(
      ins_lec6b = ifelse(ins_lec6b <= 25, 0,
      ifelse(ins_lec6b > 25 & ins_lec6b <= 50, 1, 2)),
      ins_mat6b = ifelse(ins_mat6b <= 25, 0,
      ifelse(ins_mat6b > 25 & ins_mat6b <= 50, 1, 2))), by = 'rbd')
...
remove(sep20, mat20, dir20, CDM2018, CDB2018)
```

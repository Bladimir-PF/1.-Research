...
c1_18%>%
  filter(pos_valida == 1, agregada_por_continuidad == 0, preferencia_postulante == 1)%>%
  group_by(rbd)%>%
  mutate(
    first_pref = n())%>%
  select(mrun, rbd, first_pref
...
c1_18%>%
  filter(pos_valida == 1, agregada_por_continuidad == 0, preferencia_postulante == 2)%>%
  group_by(rbd)%>%
  mutate(
    second_pref = n())%>%
  select(mrun, rbd, second_pref
...
c1_18%>%
  filter(pos_valida == 1, agregada_por_continuidad == 0, preferencia_postulante == 3)%>%
  group_by(rbd)%>%
  mutate(
    third_pref = n())%>%
  select(mrun, rbd, third_pref
...
c1_18%>%
  ...
  group_by(rbd)%>%
  mutate(
    first_pref = sum(preferencia_postulante == 3))%>%
  select(mrun, rbd, first_pref
  )%>%
  view()
...
ee18%>%
  mutate(
    fourth = demanda - first_pref - second_pref - third_pref,
    diff = demanda - first_pref - second_pref - third_pref - fourth
  )%>%
  view()

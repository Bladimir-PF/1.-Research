# Primero, cargamos los paquetes en R. Con esta funcion, los paquetes que no existan en la version
# de R se cargaran de automaticamente. Si los paquetes ya existen, solo los carga.

needed_packages = c("plyr", # para transformar variables
                    "dplyr", # para transformar variables
                    'nlme', # Para estimar modelos multinivel
                    'lmerTest', # Para opcion Satterthwaite en los DDF en lmer
                    'performance', # Para calcular ICC en lmer
                    'margins', # Para predecir los valores de la regresion en la base de datos
                    'ordinal') #Modelo multinivel para variable ordinal (Holmes, p.151)
...
registered <- select(registered,
                     MRUN, RBD,CODIGO_ENS,RAMA_EDUCACIONAL,DEPENDENCIA,CLEC_ACTUAL,PUNTAJES_PROCESO)
...
registered <- data.frame(filter(registered, PUNTAJES_PROCESO == 1 & CODIGO_ENS %in% c(310,410,510,610,710,810,910) & CLEC_ACTUAL > 149))
...
data <- select(data_recode, RBD, CLEC_ACTUAL, TypeHumanist:FamIncome_Q5) %>% arrange(RBD)
rm(data_recode)
...
data <- data %>%
  select(RBD, #ID variable
         CLEC_ACTUAL, # Dependent variable
         #Level 2 predictors
...
(cor.test(data$MomED_RE, data$CLEC_ACTUAL, method="pearson")$estimate)^2 #Total R2 relative to empty model 1
#0.2338347 Total R2
...
(cor.test(data$DadED_RE, data$CLEC_ACTUAL, method="pearson")$estimate)^2 #Total R2 relative to empty model 1
# 0.2515326 
...
(cor.test(data$FamInc_RE, data$CLEC_ACTUAL, method="pearson")$estimate)^2 #Total R2 relative to empty model 1
...
School_RI <- lmer(
  formula = CLEC_ACTUAL ~ 1 + TypeHumanist + SectorSubsidized + SectorPriv + (1|RBD),
  REML = TRUE,
  data = data)
summary(School_RI, ddf="Satterthwaite"); llikAIC(School_RI, chkREML=FALSE)

School_RI_int <- lmer(
  formula = CLEC_ACTUAL ~ 1 + TypeHumanist + SectorSubsidized + SectorPriv +
...
(cor.test(data$School_RI_int, data$CLEC_ACTUAL, method="pearson")$estimate)^2 #Total R2 relative to empty model 1
